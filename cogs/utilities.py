...
class utilities(commands.Cog, description="Handy dandy utils"):
    ...
    async def on_message_edit(self, before, after):
        ...
        self.edit_snipe_cache[before.channel.id]['before'] = [before.content, before.author.name, before.author.avatar_url, before.author.discriminator]
        self.edit_snipe_cache[before.channel.id]['after'] = [after.content, after.author.name, after.author.avatar_url, after.author.discriminator]
    ...
    async def on_message_delete(self, message):
        if message.author.bot:
            return
        self.snipe_cache[message.channel.id] = [message.content, message.author.name, message.author.avatar_url, message.author.discriminator]
        await asyncio.sleep(60)
    ...
    async def snipe(self, ctx):
        ...
        author = ctx.author 
        try:
            em = Embed(name = f"Last deleted message in #{channel.name}", 
                       description = self.snipe_cache[channel.id][0],
                       timestamp=datetime.datetime.utcnow(),
                       colour=discord.Color.random()
                       )
            em.set_author(name = f"{self.snipe_cache[channel.id][1]}#{self.snipe_cache[channel.id][3]}", icon_url=f"{self.snipe_cache[channel.id][2]}")
    ...
    async def editsnipe(self, ctx):
        ...
        author = ctx.author 
        try:
            em = Embed(name = f"Last edited message in #{channel.name}", 
                       description = f"**Before:**\n+ {self.edit_snipe_cache[channel.id]['before'][0]}\n"
                                     f"\n**After:**\n- {self.edit_snipe_cache[channel.id]['after'][0]}", 
                       timestamp=datetime.datetime.utcnow(), 
                       colour=discord.Color.random()
                       )
            em.set_author(name = f"{self.edit_snipe_cache[channel.id]['before'][1]}#{self.edit_snipe_cache[channel.id]['before'][3]}", 
    ...
    async def gstart(self, ctx, time, winners : str, *, prize):
        ...
        if grole is None:
            return await ctx.send(f"You've not set up a giveaway role for this server! Please do so with `{ctx.prefix}config grole <role>`")
        if grole not in ctx.author.roles:
            return await ctx.send(f"{self.bot.redTick} You do not have the role `{grole.name}` that is required to start a giveaway!")


        authorURL = ctx.author.avatar_url
        winners = int(winners.replace("w",""))
        if winners > 30 or winners < 1:
            raise commands.BadArgument(f"{self.bot.redTick} Max. winners is 30, min. is 1")
        ...
        timeconverter = precisedelta(delta,minimum_unit='seconds',suppress=['microseconds'])
        randomcolour = discord.Color.random()
        current_time = datetime.datetime.utcnow()
        time_added = datetime.timedelta(seconds = time1)
        future_time = current_time + time_added
        embed = Embed(title=f'{prize}', description=f'React with <a:party_tada:809202936600199209> to enter!\nEnds in **{timeconverter}**\nHosted by {ctx.author.mention}', timestamp=future_time, color=randomcolour)
        ...
        while True:
            ...
            if time1 == 0:
                ...
                try:
                    ...
                    for _ in range(winners):
                        ...
                        emb = Embed(title="You won!", description=f"You have won the giveaway for [{prize}]({msg.jump_url}) in **{guild}** - {channel.mention}", colour=discord.Color(0x4CAF50))
                        emb.set_footer(text="🎉 Congratulations!")
                        await winner.send(embed=emb)
                        winnerstr.append(winner.id)

                    if winnerstr == []:
                        em = Embed(title=f'{prize}', description=f'Could not determine a winner\nHosted by {ctx.author.mention}', timestamp=future_time, color=discord.Color(0x2F3136))
                        em.set_footer(text=f"{winners}w | Ended at ")
                        em.set_author(name="This giveaway has ended", icon_url=f"{authorURL}")
                        await msg.edit(embed=em)
                    else:
                        await ctx.send(", " .join(f"<@{users}>" for i, users in enumerate(winnerstr)) +  f" has won the giveaway for **{prize}**\n{msg.jump_url}")
                        em = Embed(title=f'{prize}', description=f"{plural1} " + ", " .join(f"<@{users}>" for i, users in enumerate(winnerstr)) + f"\nHosted by {ctx.author.mention}", timestamp=future_time, color=discord.Color(0x2F3136))
                        em.set_footer(text=f"{winners}w | Ended at ")
                        em.set_author(name="This giveaway has ended", icon_url=f"{authorURL}")
                        await msg.edit(embed=em)
                        emb1 = Embed(title=f'Your giveaway has ended!', url=msg.jump_url, description=f"[__**Desktop Friendly URL**__]({msg.jump_url})\nYou have **{winners} {plural}**\n" + "\n" .join(f"`{i + 1}` <@{users}> - [{users}]" for i, users in enumerate(winnerstr)), colour=discord.Color(0xFFF857))
            ...
            em = Embed(title=f'{prize}', description=f'React with <a:party_tada:809202936600199209> to enter!\nEnds in **{timeconverter}** \nHosted by {ctx.author.mention}', timestamp=future_time, color=randomcolour)
    ...
    @commands.command(name="reroll", brief="Rerolls a giveaway from a message ID", usage="<channel> <messageID>")
    async def reroll(self, ctx, id_:int):
        ...
        if grole not in ctx.author.roles:
            return await ctx.send(f"{self.bot.redTick} You do not have the role `{grole.name}` that is required to reroll a giveaway!")

        try:
            new_msg = await channel.fetch_message(id_)
        except:
            raise commands.BadArgument("The message with the given ID is not found in the current channel.")
        users = await new_msg.reactions[0].users().flatten()
        users.pop(users.index(self.bot.user))
    ...
    @commands.group(invoke_without_command=True, case_insensitive=True, brief="A special form of giveaway.") 
    ...
    async def custom(self, ctx, *, prize):
        ...
        if grole not in ctx.author.roles:
            return await ctx.send(f"{self.bot.redTick} You do not have the role `{grole.name}` that is required to start a drop!")
        rarity = random.choice(['<:green:819177876216610838>', '<:red:819177626278297620>', '<:purple:819177770793697280>'])
        def check1(m):
            return m.author == ctx.author
        try:
            await ctx.send(f"{ctx.author.mention}, please answer my question in your DM")
            ...
            await ctx.author.send(f"Ok, the sentence will now be `{msg.content}`.\nYou can now go back to {ctx.channel.mention}")
        ...
        while True:
            ...
            if msg.content == str(sentence):
                await msgg.edit(content=f"{rarity} **`EVENT TIME!`**\n**Type it, type it, type it!**\nType the given sentence as fast as possible.\n\n<:red:819177626278297620> `This event has expired. No new submissions will be accepted`")
                await ctx.send(content=f'{msg.author.mention} won **{prize}**! — Claim your prize from {ctx.author.mention}')
                break
    ...
    async def normal(self, ctx, prize, time=None):
        ...
        if time:
            ...
            msg1 = await ctx.send(content=f"{rarity} **`EVENT TIME!`**\n**Type it, type it, type it!**\nType the given sentence as fast as possible.\n\nLanding in **{time}** seconds.")
            while True:
                if time < 0:
                    break
                async with ctx.channel.typing():
                    time -= 1
                    if time == 0:
                        await msg1.edit(content=f"{rarity} **`EVENT TIME!`**\n**Type it, type it, type it!**\nType the given sentence as fast as possible.\n\nLanding NOW!!!")
                        r = RandomWord()
                        s = RandomSentence()
                        ...
                        while True:
                            try:
                                msg = await self.bot.wait_for('message', timeout=25, check=check)
                            except asyncio.TimeoutError:
                                await ctx.send("Nobody won...")
                            if msg.content == str(sentence):
                                await msg1.edit(content=f"{rarity} **`EVENT TIME!`**\n**Type it, type it, type it!**\nType the given sentence as fast as possible.\n\n<:red:819177626278297620> `This event has expired. No new submissions will be accepted`")
    ...
    @commands.command(name="ui", aliases=["info", "whois"], brief="Displays an user's information")
    async def ui(self, ctx, member: discord.Member = None):
        ...
        em = Embed(title="", description=f"{member.mention}", color=guild.me.colour, timestamp=datetime.datetime.utcnow())
        em.add_field(name="Joined", value=member.joined_at.strftime("%a, %b %d, %Y %H:%M %p"))
        em.add_field(name="Created", value=member.created_at.strftime("%a, %b %d, %Y %H:%M %p"))
        roles = member.roles[1:]

        if roles:
            em.add_field(name=f"Roles [{len(roles)}]", value=" ".join(f"{role.mention}" for role in roles), inline=False)
        else:
            em.add_field(name=f"Roles [{len(roles)}]", value="This member has no roles", inline=False)
    ...
    @commands.command(brief="Previews a hex color (format is #FFFFFF)", usage="<hexadecimal>")
    async def hex(self, ctx, *, args=None):
        ...
        if args:
            ...
            args1 = "0x" + args1
            try:
                colour = await ColourConverter().convert(ctx, args1)
            except:
                raise discord.ext.commands.MissingRequiredArgument(ctx.command)
            em = Embed(description=f"Color `{args}`", colour=colour)
            em.set_author(name="Hex Viewer")
            em.set_thumbnail(url=f'https://some-random-api.ml/canvas/colorviewer?hex={args2}')
            return await ctx.send(embed=em)
        else:
            ...
            em.set_author(name="Hex Viewer")
            em.set_thumbnail(url=f'https://some-random-api.ml/canvas/colorviewer?hex={hex}')
            return await ctx.send(embed=em)
