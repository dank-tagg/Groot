...
class currency(commands.Cog):
    ...
    async def cog_before_invoke(self, ctx):
        ...
        if await self.data.create_account(ctx.author.id) is True:
            ctx.bucket.reset()
            raise commands.BadArgument("Seems like you are new! I created an account for you.")
        if ctx.author.id not in self.bot.cached_users:
            ...
            self.bot.cached_users[ctx.author.id] = {"wallet": r[1], "bank": r[2], "max_bank": r[3], "boost": r[4], "exp": r[5], "lvl": r[6], "prestige": r[7]}
    ...
    @commands.command(name="profile", aliases=["lvl"], brief="Shows your stats and level")
    ...
    async def _profile(self, ctx, member: discord.Member=None):
        ...
        if member.id not in self.bot.cached_users:
            return await ctx.maybe_reply(f"{self.bot.redTick} That user does not have an account yet!")
        ...
        em = Embed()
        em.add_field(name="Level", value=f"**`{lvl}`**\n{link(progress_bar(lvl), f'{discord.utils.oauth_url(812395879146717214)}')}")
        em.add_field(name="Experience", value=f"**`{exp}`**\n"
                                              f"{link(progress_bar(exp-lvl*100), f'{discord.utils.oauth_url(812395879146717214)}')}")
        em.add_field(name="Cash", value=f"**Wallet**: ⛻{wallet_amount:,}\n**Bank**: ⛻{bank_amount:,}\n**Net worth**: ⛻{wallet_amount+bank_amount:,}")
        em.add_field(name="Misc", value=f"`{data[0]:,}` commands issued")
        em.set_author(name=f"{member.display_name}'s profile", icon_url=member.avatar_url)
        await ctx.send(embed=em)
        
    @commands.command(name="prestige")
    @commands.check(grootCooldown(1, 1*60*60*24, 1, 1*60*60*24, commands.BucketType.user))
    async def _prestige(self, ctx):
        ...
        if check:
            ...
            return await ctx.send(f":tada: Congratulations {ctx.author.mention}! You are now Prestige `{roman_num(prestige+1)}`.\n"
                                  "You've earned a **25%** multiplier, and a redeemable PREMIUM PASS!")
        else:
            
            raise commands.BadArgument(f"You do not have enough funds to prestige!\nYou need `{prestige*15-level}` more levels.")
    ...
    async def _balance(self, ctx, member: discord.Member=None):
        ...
        em = Embed(title=f"{member.display_name}'s balance", 
        ...
        em.set_footer(text=random.choice(["ew poor", "imagine being poor", "sucks to suck", "lmfaooo", "nice balance"]))
    ...
    async def _inventory(self, ctx):
        ...
        em = Embed(description=inventory if inventory else "No items to see here...").set_author(name=f"{ctx.author.display_name}'s inventory")
    ...
    async def _buy(self, ctx, amount:typing.Optional[int]=1, *, item):
        ...
        if await self.data.get_data(ctx.author.id) < data[0]*amount:
            raise commands.BadArgument(f"{ctx.author.mention} You do not have enough money for this purchase!")
        ...
        em = Embed(description=f"Successfully bought `{amount}` `{item}` for **⛻{data[0]*amount:,}**")
    ...
    async def _shop(self, ctx, item=None):
        if item:
            ...
            data = await cur.fetchone()
            if not data:
                raise commands.BadArgument(f"{item} is not an recognized item. Please check your spelling.")
            em = Embed(title=data[0], description=data[1])
            em.add_field(name="Value", value=f"**BUY**: ⛻{data[2]:,}\n**SELL**: ⛻{round(data[2]*0.25):,}")
            return await ctx.send(embed=em)
        else:
            query = """
                    SELECT item_name, item_description, item_price
    ...
    async def _sell(self, ctx, amount:typing.Optional[int]=1, *, item):
        ...
        if amount > data[1]:
            raise commands.BadArgument(f"{ctx.author.mention} You do not have `{amount:,}` {item} to sell! You only have `{data[1]}`")
        item = data[2]
        query = """
        ...
        em = Embed(description=f"Successfully sold `{amount}` `{item}` for **⛻{int(data[0]*amount*0.25):,}**")
        em.set_author(name=ctx.author.display_name, icon_url=ctx.author.avatar_url)
        return await ctx.send(embed=em)
        
    @commands.command(name="deposit", aliases=["dep"], brief="Deposit money from your wallet.")
    @commands.check(grootCooldown(1, 5, 1, 1, commands.BucketType.user))
    async def _deposit(self, ctx, amount:str):
        ...
        if amount == 0:
            raise commands.BadArgument(f"{ctx.author.mention} You have no coins in your wallet.")
        if amount > wallet or amount < 1:
            raise commands.BadArgument(f"{ctx.author.mention} Your argument must be a positive number and cannot be more than you own (⛻{wallet:,})!")
        amount = amount if amount + bank < max_bank else max_bank - bank
        if amount == 0:
            raise commands.BadArgument(f"You can only hold **⛻{max_bank:,}** in your bank right now.")
        
        await self.data.update_data(ctx.author.id, amount, mode="bank")
        await self.data.update_data(ctx.author.id, -amount)
        return await ctx.send(f"{ctx.author.mention} You deposited **⛻{amount:,}** into your bank. "
    ...
    async def _withdraw(self, ctx, amount:str):
        ...
        if amount > bank or amount < 1:
            raise commands.BadArgument(f"{ctx.author.mention} Your argument must be a positive number and cannot be more than you own (⛻{bank:,})!")
        await self.data.update_data(ctx.author.id, -amount, mode="bank")
        await self.data.update_data(ctx.author.id, amount)
        return await ctx.send(f"{ctx.author.mention} You withdrew **⛻{amount:,}** from your bank. "
                              f"Now you have **⛻{await self.data.get_data(ctx.author.id, mode='bank'):,}**")
    ...
    async def _fish(self, ctx, info=None):
        ...
        if not await self.bot.data.has_item(ctx.author.id, "fishing rod"):
            raise commands.BadArgument(f"{self.bot.redTick} You need a `Fishing Rod` to use `fish`!")
        boost = self.bot.cached_users[ctx.author.id]['boost']
        times_caught = random.randint(1, 3)

        fish_dict = {"🐟 Common Fish": 100, "🐡 Blow Fish": 300, "🐠 Tropical Fish": 700, "🦈 Shark": 1200, "🦑 Squid": 1000, "🦀 Crab": 2000}
        randomized = {k: fish_dict[k] for k in random.sample(fish_dict.keys(), times_caught)}
        if info is not None and info == "info":
            fish_dict["<:Groot:829361863807860756> Groot"] = 100000
            fish_list = [f"{k} | **⛻{v:,}**" for k, v in sorted(fish_dict.items(), key=lambda item: item[1])]
        ...
        fish_caught_alpha = [random.choice(list(randomized.keys())) for _ in range(times_caught)]
        fish_value = sum([randomized[fish] for fish in fish_caught_alpha])
        fish_caught = [f"{fish_caught_alpha.count(item)} {item}" for item in fish_caught_alpha]
        ...
        txt = f"Multiplier Bonus +{round(boost*100)-100}% (**`⛻{int(fish_value*boost-fish_value):,}`**)" if boost > 1 else ""
        em = Embed(description=f"You went to fish and caught...\n\n" +
                               '\n'.join(fish_caught) + 
                               f"\n\nYou sold them for **⛻{fish_value:,}**\n" +
                               txt
                  )
        
        em.set_author(name=f"{ctx.author.display_name}'s fishing trip", icon_url=ctx.author.avatar_url)
    ...
    async def _hunt(self, ctx, info=None):
        ...
        boost = self.bot.cached_users[ctx.author.id]['boost']
        times_caught = random.randint(1, 3)
        animals_dict = {"🦌 Deer": 1200, "🐗 Boar": 1000, "🐰 Rabbit": 750, "🐓 Chicken": 500}
        randomized = {k: animals_dict[k] for k in random.sample(animals_dict.keys(), times_caught)}
        if info is not None and info == "info":
            animals_dict["🦄 Unicorn"] = 25000
            animals_dict["🐲 Dragon"] = 50000
            animals_dict["<:Groot:829361863807860756> Groot"] = 100000
            animals_list = [f"{k} | **⛻{v:,}**" for k, v in sorted(animals_dict.items(), key=lambda item: item[1])]
            em = Embed(description="\n".join(animals_list))
        ...
        animals_caught_alpha = [random.choice(list(randomized.keys())) for _ in range(times_caught)]
        animals_value = sum([randomized[animal] for animal in animals_caught_alpha])
        animals_caught = [f"{animals_caught_alpha.count(item)} {item}" for item in animals_caught_alpha]
        animals_caught = list(OrderedDict.fromkeys(animals_caught))
        earnings = round(animals_value * boost)
        await self.data.update_data(ctx.author.id, earnings)
        txt = f"Multiplier Bonus +{round(boost*100)-100}% (**`⛻{int(animals_value*boost-animals_value):,}`**)" if boost > 1 else ""
        em = Embed(description=f"You went to the woods and caught...\n\n" +
                               '\n'.join(animals_caught) + 
                               f"\n\nYou sold them for **⛻{animals_value:,}**\n" +
                               txt
                               )
        em.set_author(name=f"{ctx.author.display_name}'s hunt", icon_url=ctx.author.avatar_url)
        em.set_footer(text=f"Current Multiplier: {round(boost*100)}%")
        return await ctx.maybe_reply(content=ctx.author.mention, embed=em)
    ...
    async def _give(self, ctx, amount, member: discord.Member):
        ...
        if member == ctx.author or member.bot:
            raise commands.BadArgument(f"{self.bot.redTick} You cannot share coins to yourself or a bot.")
        
        amount = await convert_to_int(amount, await self.data.get_data(ctx.author.id))
        if amount < 1:
            raise commands.BadArgument(f"{self.bot.redTick} Amount must be a positive number!")
        ...
        return await ctx.maybe_reply(f"{ctx.author.mention} You gave **⛻{int(amount_shared):,}** "
    ...
    async def _slots(self, ctx, amount:str):
        ...
        wallet = await self.data.get_data(ctx.author.id)
        if wallet >= 10000000:
            raise commands.BadArgument(f"{ctx.author.mention} You are too rich to gamble!")
        boost = self.bot.cached_users[ctx.author.id]['boost']
        if wallet == 0:
            raise commands.BadArgument(f"{ctx.author.mention} You have no coins to gamble with.")
        amount = await convert_to_int(amount, 500000)
        if amount > 500000:
            raise commands.BadArgument(f"{ctx.author.mention} You can't slots more than ⛻500,000 coins")
        if amount > wallet:
            if amount != 500000:
                raise commands.BadArgument(f"{ctx.author.mention} You don't have that much coins!")
            else:
                amount = wallet
        #Emojis
        emojis = [':four_leaf_clover:', ':cherry_blossom:', ':wood:', ':shell:', ':maple_leaf:', ':fish:', ':octopus:', ':crab:', ':star:']
        result = random.choices(emojis, k=3)
        #Set vars to keep it simple
        triple = result[0] == result[1] == result[2]
        double = result[0] == result[1] or result[1] == result[2] or result[0] == result[2]
        ...
        em = Embed(title="",
                   description=
                  f"You {won_or_lost} **⛻{abs(winnings):,}**\n"
        ...
        em.set_author(name=f"{ctx.author.display_name}'s slots table", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=em)
    ...
    async def levels(self):
        ...
        for user in self.cache:
            ...
            if await self.data.get_data(user, mode="exp") > (lvl := await self.data.get_data(user, mode="lvl")+1)*100:
                await self.data.update_data(user, 1, mode="lvl")
                await self.data.update_data(user, 0.01, mode="boost")
                self.bot.cached_users[user]['boost'] = round(await self.data.get_data(user, mode="boost"), 2)
        self.cache = {}
