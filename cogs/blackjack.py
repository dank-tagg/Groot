...
class blackjack(commands.Cog):
    ...
    async def play_blackjack(self, ctx, amount):
        ...
        stood = False
        won = None
        deck = [{"face": face, "suit": suit} for suit in self.suits for face in self.faces]
        cards = {'user_cards': [self.deal(deck), self.deal(deck)], 'bot_cards': [self.deal(deck), self.deal(deck)]}
        
        while self.total_value(cards['user_cards']) >= 21 or self.total_value(cards['bot_cards']) >= 21:
            cards = {'user_cards': [self.deal(deck), self.deal(deck)], 'bot_cards': [self.deal(deck), self.deal(deck)]}
        ...
        while fail < 2:
            ...
            if not isinstance(status, int):
                ...
                try:
                    await ctx.send(content="What do you want to do?\nType `h` to hit, `s` to stand, `e` to end.", embed=self.start(ctx, cards))
                    ...
                    if msg == 'h':
                        ...
                        return await ctx.maybe_reply("You ended the game. Half of your bet was lost.")
                    else:
                        fail += 1
        await self.data.update_data(ctx.author.id, -round(0.5*amount))
        return await ctx.maybe_reply("You lost the game due to multiple invalid choices. Half of your bet was lost.")
    ...
    def score(self, stood, user_cards, bot_cards):
        if self.total_value(user_cards) > 21:
            ...
            return {'result': False, 'message': 'You lose! Your opponent reached 21 before you!'}
            ...
            return {'result': True, 'message': 'You won! You took 5 cards without going over 21.'}
        elif self.total_value(bot_cards) < 21 and len(bot_cards) == 5:
            return {'result': False, 'message': 'You lose! Your opponent took 5 cards without going above 21.'}
        elif (bot_deck := self.total_value(bot_cards)) > (user_deck := self.total_value(user_cards)) and stood:
            return {'result': False, 'message': f'You lose! You have {user_deck}, Dealer has {bot_deck}.'}
        elif (user_deck := self.total_value(user_cards)) > (bot_deck := self.total_value(bot_cards)) and stood:
            return {'result': True, 'message': f'You won! You have {user_deck}, Dealer has {bot_deck}.'}
        else:
            return self.total_value(user_cards)
    
    def start(self, ctx, cards):
        user_cards_visual = "".join([link(f"**`{self.get_icon(card['suit'])} {card['face']}`** ", discord.utils.oauth_url(812395879146717214)) for card in cards['user_cards']])
        bot_cards_visual = [link(f"**`{self.get_icon(card['suit'])} {card['face']}`** ", discord.utils.oauth_url(812395879146717214)) for card in cards['bot_cards']]
        em = Embed()
        em.add_field(name=ctx.author.display_name, value=f"Cards - {user_cards_visual}\nValue - `{self.total_value(cards['user_cards'])}`")
        em.add_field(name=self.bot.user.display_name, value=f"Cards - {bot_cards_visual[0]} `?`\nValue - ` ? `")
    ...
    async def end(self, ctx, cards, kwargs:dict):
        ...
        won_or_lost = f"won  **⛻{ctx.amount:,}**! " if result is True else f"lost **⛻{ctx.amount:,}**. "
        ...
        user_cards_visual = "".join([link(f"**`{self.get_icon(card['suit'])} {card['face']}`** ", discord.utils.oauth_url(812395879146717214)) for card in cards['user_cards']])
        bot_cards_visual = "".join([link(f"**`{self.get_icon(card['suit'])} {card['face']}`** ", discord.utils.oauth_url(812395879146717214)) for card in cards['bot_cards']])
        em = Embed(description=message, color=color)
        em.add_field(name=ctx.author.display_name, value=f"Cards - {user_cards_visual}\nValue - `{self.total_value(cards['user_cards'])}`")
        em.add_field(name=self.bot.user.display_name, value=f"Cards - {bot_cards_visual}\nValue - `{self.total_value(cards['bot_cards'])}`")
        em.set_author(name=ctx.author.display_name, icon_url=ctx.author.avatar_url)
        em.set_footer(text="K, Q, J = 10  |  A = 1 or 11")
        return em
    ...
    async def dealersTurn(self, ctx, cards, deck):
        if self.total_value(cards['user_cards']) > 21:
            return await self.end(ctx, cards, self.score(True, cards['user_cards'], cards['bot_cards']))
