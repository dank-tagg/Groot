...
class Moderator(commands.Cog):
        ...
        *,
        ...
        if mode != "remove" and mode != "add":
            return await ctx.send(
                f"{self.bot.emojis_dict('redTick')} Accepted values are `add/remove` for `mode`"
            )
    ...
    async def _edit_(
        ...
        return await ctx.send(
            f"{self.bot.emojis_dict('greenTick')} Successfully gave {user.mention} {amount:,} `{action}`."
        )
    ...
    async def _delete_item_from_shop(self, ctx, *, item):
        ...
        await self.bot.db.commit()
        return await ctx.send(f"{self.bot.emojis_dict('greenTick')} Deleted item `{item}` from shop.")
