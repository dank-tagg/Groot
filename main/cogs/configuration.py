...
from discord.ext.commands import (MemberConverter, TextChannelConverter,
                                  command, guild_only, has_guild_permissions)
...
class configuration(commands.Cog):
    ...
    async def _tips(self, ctx, *, mode:str):
        ...
        return await ctx.send(f"{self.bot.greenTick} Toggled your tips to `{mode.upper()}`")



    @commands.group(invoke_without_command=False, case_insensitive=True, brief="Edit server settings")
    @guild_only()
    ...
    async def config(self, ctx):
        ...
        """
        finder = await self.bot.db.execute("SELECT * FROM guild_config WHERE guild_id=?", (ctx.guild.id,))
        row = await finder.fetchone()
        if row is None:
            query = "INSERT INTO guild_config (guild_id) VALUES (?)"
            await self.bot.db.execute(query, (ctx.guild.id,))
            await ctx.send(f"Seems like you are new! I added this server ({ctx.guild.name}), to our database. Enjoy!")
    ...
    async def _setprefix(self, ctx, *, prefix:str):
        ...
        await ctx.send(f"The prefix has been set to `{prefix}`. To change the prefix again, use `{prefix}config prefix <prefix>`")
    
    @commands.command(name="disable", brief="Disables a command for a server or channel", usage="[channel] <command>")
    async def _disable(self, ctx, snowflake_id: typing.Optional[discord.TextChannel], *, command):
        """
        ...
        txt = f" for {snowflake_id.mention}" if isinstance(snowflake_id, discord.TextChannel) else " for this server"
        try:
            await self.bot.db.execute(query, (snowflake_id.id, command))
        except:
            raise commands.BadArgument(f"{self.bot.redTick} That command is already disabled{txt}!")
        else:
            await self.bot.db.commit()
            try:
    ...
    @commands.command(name="enable", brief="Enables a disabled command for a server or channel", usage="[channel] <command>")
    async def _enable(self, ctx, snowflake_id: typing.Optional[discord.TextChannel], *, command):
        """
        ...
        txt = f" for {snowflake_id.mention}" if isinstance(snowflake_id, discord.TextChannel) else " for this server"
        cur = await self.bot.db.execute("SELECT command_name FROM disabled_commands WHERE snowflake_id = ? AND command_name = ?", (snowflake_id.id, command))
        row = await cur.fetchone()
        
        if row is None:
            raise commands.BadArgument(f"{self.bot.redTick} That command is not disabled{txt}!")
        else:
