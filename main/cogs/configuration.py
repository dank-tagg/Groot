...
class Configuration(commands.Cog):
    ...
    async def _tips(self, ctx, *, mode: str):
        ...
        return await ctx.send(
            f"{self.bot.emojis_dict('greenTick')} Toggled your tips to `{mode.upper()}`"
        )
    ...
    async def _disable(
        ...
        try:
            ...
            raise commands.BadArgument(
                f"{self.bot.emojis_dict('redTick')} That command is already disabled{txt}!"
            )
        else:
            await self.bot.db.commit()
            try:
            ...
            await ctx.send(f"{self.bot.emojis_dict('greenTick')} Disabled command `{command}`{txt}")
    ...
    async def _enable(
        ...
        if row is None:
            raise commands.BadArgument(
                f"{self.bot.emojis_dict('redTick')} That command is not disabled{txt}!"
            )
        else:
            ...
            await self.bot.db.execute(query, (snowflake_id.id, command))
            await self.bot.db.commit()
            await ctx.send(f"{self.bot.emojis_dict('greenTick')} Enabled command `{command}`{txt}")
