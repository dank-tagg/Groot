...
class fun(commands.Cog, description="Fun commands"):
    ...
    async def gtn(self, ctx):
        ...
        await ctx.send(f"A number between **1 and 10** has been chosen, You have 3 attempts to guess the right number! Type your guess in the chat as a valid number!")
        for i in range(0, 3):
            try:
                response = await self.bot.wait_for('message', timeout=10, check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
                guess = int(response.content)
                
                if guess > 10 or guess < 1:
                    if 2-i == 0:
                        await ctx.send(f'Unlucky, you ran out of attempts. The number was **{no}**')
                        return
                    else:
                        await ctx.send("That is not a valid number! It costed you one attempt...")
                    ...
                    if guess > no:
                        if 2-i == 0:
                            ...
                            await ctx.send(f'The number is smaller than {guess}\n`{2-i}` attempts left')
                    elif guess < no:
                        if 2-i == 0:
                            await ctx.send(f'Unlucky, you ran out of attempts. The number was **{no}**')
                            return
                        else:
                            await ctx.send(f'The number is bigger than {guess}\n`{2-i}` attempts left')
                        
                    else:
                        await ctx.send(f'Good stuff, you got the number right. I was thinking of **{no}**')
                        return
    ...
    async def gayrate(self, ctx, member: discord.Member = None):
        ...
        if member:
            emb = Embed(title="gay r8 machine", description=f"{member.name} is {random.randrange(0, 100)}% gay 🌈", color=discord.Color.random())
            await ctx.send(embed=emb)
        else:
            emb = Embed(title="gay r8 machine", description=f"You are {random.randrange(0, 100)}% gay 🌈", color=discord.Color.random())
            await ctx.send(embed=emb)

    @gayrate.error
    async def gayrate_error(self, ctx, error):
        if isinstance(error, commands.MemberNotFound):
            emb = Embed(title="gay r8 machine", description=f"{error.argument} is {random.randrange(0, 100)}% gay 🌈", color=discord.Color.random())
            await ctx.send(embed=emb)
    ...
    async def eightball(self, ctx, *, question):
        ...
        answers = ["It is certain.", "It is decidedly so.", "Without a doubt.", "Is Trump's skin orange?", "Definitely", "Why don't you go ask your mom smh.", "What? No!", "Unscramble `esy`", "Doubtful...", "I'm lazy rn, don't want to answer it.", "Ok, no", "Possibly so!", "Yes. Yes. Yes."]
        if question:
            em = Embed(title="Magic 8-ball", description=f"You: {question}\n🎱: {random.choice(answers)}", colour=discord.Color.random())
            await ctx.send(embed=em)

    @commands.group(invoke_without_command=True, case_insensitive=True, usage="<encode | decode>")
    ...
    async def encode(self, ctx, *, text):
        ...
        async with binary.get(f"https://some-random-api.ml/binary?text={text}") as texxt:
    ...
    async def decode(self, ctx, *, binary):
        ...
        async with decode.get(f"https://some-random-api.ml/binary?decode={binary}") as s:
            res = await s.json()
            decoded = res['text']
    ...
    async def fight(self, ctx, member: discord.Member):
        ...
        while True:
            if user1_hp <= 0 or user2_hp <= 0:
                ...
                await ctx.send(random.choice([f"Wow! **{winner.name}** totally melted down **{loser.name}**, winning with just `{winner_hp} HP` left!",
            ...
            await ctx.send(f"{alpha.mention}, what do you want to do? `punch`, `kick`, `slap` or `end`?\nType your choice out in chat as it's displayed!")
            ...
            try:
                ...
                await ctx.send(f"**{alpha.name}** didn't react on time. What a noob. **{beta.name}** wins!")
                return

            if msg.content.lower() == 'punch':
                damage = random.choice([random.randint(20,60), random.randint(0,50), random.randint(30,70),random.randint(0,40),random.randint(10,30) ,random.randint(5,10)])
                ...
                randommsg = random.choice([f'**{alpha.name}** deals **{damage}** damage with an OP punch.\n**{beta.name}** is left with {hpover} HP', 
                ...
                damage = random.choice([random.randint(30,45), random.randint(30,60), random.randint(-50,-1), random.randint(-40,-1)])
                if damage > 0:
                    ...
                    await ctx.send(random.choice([f"**{alpha.name}** kicks **{beta.name}** and deals **{damage}** damage\n**{beta.name}** is left over with **{hpover}** HP",
                    ...
                    await ctx.send(random.choice([f"**{alpha.name}** flipped over while kicking their opponent, dealing **{-damage}** damage to themselves.", 
                ...
                await ctx.send(f"**{alpha.name}** slaps their opponent, and deals **{damage}** damage.\n{beta.name} is left over with **{hpover}** HP")

            elif msg.content.lower() == 'end':
                await ctx.send(f"{alpha.name} ended the game. What a pussy.")
                return

            elif msg.content.lower() != 'kick' and msg.content.lower() != 'slap' and msg.content.lower() != 'punch' and msg.content.lower() != 'end':
                if fails_user1 >= 1 or fails_user2 >= 1:
                    return await ctx.send("This game has ended due to multiple invalid choices. God ur dumb")
                if alpha == user1:
                    fails_user1 += 1
                else:
                    fails_user2 += 1
