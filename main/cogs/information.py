...
class information(commands.Cog):
    ...
    async def invite(self, ctx):
        ...
        em = Embed(title=f"Invite {self.bot.user.name} to your server!",
                           color=0x2F3136)
        em.add_field(name=f"Invite the bot", 
    ...
    async def create_menu(self, ctx: commands.Context, cog:str):
        ...
        if self.bot.get_cog(cog) == None:
            return await ctx.reply(f"No command/category found. Try using `{ctx.prefix}help` to see all available commands.", mention_author=False)
        ...
        for command in enumerate(allcommands[start:end], start=start):
            ...
            embed.add_field(name='{} {}'.format(command, command.signature), value='{}'.format(command.brief), inline=False)
        msg = await ctx.send(embed=embed)
        ALLEMOJIS = ['\U000023ea', '\U000025c0', '<:trashcan:822050746333003776>', '\U000025b6', '\U000023e9']
        for e in ALLEMOJIS:
        ...
        while True:
            ...
            def check(reaction, m):
                return m == ctx.author and str(reaction.emoji) in ALLEMOJIS and reaction.message == msg
            try:
    ...
    @commands.command(name="help", brief="Shows help for the bot", usage="[command|cog]")
    async def _help(self, ctx, *,command:str=None):
        ...
        if command:
            if self.bot.get_command(command) == None:
                ...
                if f != None:
                    ...
                    await ctx.reply(f"No command/category found. Try using `{ctx.prefix}help` to see all available commands.", mention_author=False)

            command = self.bot.get_command(command)
            em = Embed(title=f"{command} {command.signature}", color=0x2F3136, description=f"{command.help}")

            if command.aliases:
                aliases = ", ".join(command.aliases)
            else:
                aliases = "No aliases"
            default = discord.utils.find(lambda c:isinstance(c, grootCooldown), command.checks).default_mapping._cooldown.per
            altered = discord.utils.find(lambda c:isinstance(c, grootCooldown), command.checks).altered_mapping._cooldown.per
            if default is not None and altered is not None:
                em.add_field(name="Cooldowns", value=f"Default: `{default}s`\nPremium: `{altered}s`")
            ...
            try:
                for sub in command.walk_commands():
                    ...
                    subs += (f"`{str(sub).replace(' ', '` `')}` `{str(sub.signature).replace(' ', '` `')}` — {sub.brief}\n" if sub.signature else f"`{str(sub).replace(' ', '` `')}` — {sub.brief}\n")
            except:
                pass
            if subs != '':
            ...
            return
        allcogs = ['• <:infosymbol:813730829989576714> **Information**',
                '🔧 **Configuration**',
                ':gear: **Moderation**',
        ...
        allemojis = ['<:infosymbol:813730829989576714>',
                '🔧',
                '⚙️',
                '⚒️',
                '🥳',
        ...
        date_time_obj = datetime.datetime.strptime(updates['upDATE'], '%Y-%m-%d %H:%M:%S.%f')
        ...
        f.add_field(name=f"📰 Latest News - {upDATE}", 
        ...
        f.set_footer(text='You can also click on one of the reactions '
        ...
        def check(reaction, m):
            return m == ctx.message.author and str(reaction.emoji) in allemojis and reaction.message == msg
        try:
    ...
    async def _uptime(self, ctx):
        ...
        """
        em = Embed(description=f"{humanize.precisedelta(datetime.datetime.utcnow() - self.bot.launch_time, format='%.0f')}", colour=0x2F3136)
