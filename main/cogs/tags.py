...
class Tags(commands.Cog):
    ...
    async def convert_tag(self, ctx, tag, content):
        ...
        if re.match(r"<[#@](!?&)(\d+)>", tag):
            raise commands.BadArgument(
                f"{self.bot.redTick} The tag name cannot be a mention!"
            )
        first_word = tag.partition(" ")[0]
        root = self.bot.get_command("tag")
        if first_word in root.all_commands:
            raise commands.BadArgument(
                f"{self.bot.redTick} This tag name starts with a reserved word."
            )

        elif len(tag) > 32 or len(content) > 2000:
            raise commands.BadArgument(
                f"{self.bot.redTick} There are limits for tag name `(32 letters)` and content `(2000 letters)`!"
            )
    ...
    async def delete(self, ctx, tag):
        ...
        if not bypass_owner_check and ctx.author.id != data[0]:
            return await ctx.send(
                f"{self.bot.redTick} You are not the owner of tag `{tag}`."
            )
