...
class Tags(commands.Cog):
    ...
    async def aaa(self, ctx, tag):
        ...
        if not row:
            cur = await self.bot.db.execute("SELECT tag_name FROM tags WHERE tag_guild_id = ? AND tag_name LIKE ?", (ctx.guild.id,f"%{tag}%"))
    ...
    async def convert_tag(self, ctx, tag, content):
        ...
        tag = tag.lower()
        if re.match(r'<[#@](!?&)(\d+)>', tag):
            raise commands.BadArgument(f'{self.bot.redTick} The tag name cannot be a mention!')
        first_word = tag.partition(' ')[0]
        root = self.bot.get_command('tag')
        if first_word in root.all_commands:
            raise commands.BadArgument(f'{self.bot.redTick} This tag name starts with a reserved word.')
        
        elif len(tag) > 32 or len(content) > 2000:
            raise commands.BadArgument(f"{self.bot.redTick} There are limits for tag name `(32 letters)` and content `(2000 letters)`!")
            
        else:
            return (await commands.clean_content().convert(ctx, tag), await commands.clean_content().convert(ctx, content))
    ...
    async def create(self, ctx, tag, content):
        ...
        try:
            cur = await self.bot.db.execute(query, (ctx.guild.id, tag, content, ctx.author.id, 0, time.time()))
        except sqlite3.IntegrityError:
            await ctx.send(f"{self.bot.redTick} That tag already exists!")
        else:
            await self.bot.db.commit()
            return await ctx.send(f"{self.bot.greenTick} Done! Created tag **{tag}**. `{await self.bot.get_prefix(ctx.message)}tag {tag}`")
    ...
    async def delete(self, ctx, tag):
        ...
        if data is None:
            return await ctx.send(f"{self.bot.redTick} No tag is found called `{tag}`.")
        ...
        if not bypass_owner_check and ctx.author.id != data[0]:
            return await ctx.send(f"{self.bot.redTick} You are not the owner of tag `{tag}`.")
        
        query = 'DELETE FROM tags WHERE tag_name = ? AND tag_guild_id = ?'
        cur = await self.bot.db.execute(query, (tag, ctx.guild.id))
        await self.bot.db.commit()
        return await ctx.send(f"{self.bot.greenTick} Deleted tag `{tag}`.")
