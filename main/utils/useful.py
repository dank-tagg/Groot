...
class currencyData:
    ...
    async def create_account(self, user_id):
        ...
        try:
            ...
            self.bot.cached_users.setdefault(user_id, {'wallet': 200, 'bank': 200, 'max_bank': 200, 'boost': 1, 'exp': 0, 'lvl': 0})
            return True
...
async def get_grole(self, ctx):
    ...
    cur = await self.bot.db.execute("SELECT grole FROM guild_config WHERE guild_id=?", (ctx.guild.id,))
    data = await cur.fetchone()
    return data[0]

async def get_frozen(self, guild: discord.Guild, member: discord.Member):
    cur = await self.bot.db.execute("SELECT * FROM frozen_names WHERE guild_id = ? AND user_id = ?", (guild.id,member.id))
...
class MemberConvert(commands.Converter):
    async def convert(self, ctx, argument):
        try:
            ...
            m = discord.utils.find(lambda x: x.name.lower() == argument.lower(), ctx.guild.members)
            if m is None:
                raise commands.BadArgument(f"{argument} is not a valid member or member ID")
            

        if not can_execute_action(ctx, ctx.author, m):
            raise commands.BadArgument(f'{ctx.bot.redTick} You cannot do this action on this user due to role hierarchy.')
        return m